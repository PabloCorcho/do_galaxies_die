#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Compute plane dlog(ssfr)(ssfr|M)/dlog(ssfr) for each simulation suite."""
...
data_sets_names = ['MAG500', 'IllustrisTNG300', 'IllustrisTNG100',
                   'SIMBA150', 'SIMBA75', 'SIMBA35',
                   'EAGLE100', 'EAGLE50', 'EAGLE25']

if not isdir('simu_data/derived_data'):
...
for ith, data_i in enumerate(data_sets):
    ...
    hdu.writeto('simu_data/derived_data/'
                + data_sets_names[ith] + '_planes.fits',
                overwrite=True)
