...
ssfr_gama, dssfr = np.loadtxt('obs_data/derived_data/gama_prior_ssfr.txt', usecols=(0,1),
                              unpack=True)
...
ax2.fill_between(ssfr_gama, np.cumsum(dssfr*ssfr_distri_gama), color='k', alpha=0.3,
                 label='GAMA')    
...
for ith, data_i in enumerate(data_sets_names.keys()):
    ...
    H, xbins = np.histogram(np.log10(sSFR), range=[-16, -7], bins=50, density=True)
